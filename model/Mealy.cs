...
namespace automata_equivalente.model
{
    ...
    class Mealy
    {
        ...
        public Mealy(string[] sigma, string[] rows) {
            rowsNumber = rows.Length + 1;
            columnNumber = sigma.Length + 1;
            table = new string[rows.Length + 1, sigma.Length + 1];
        ...
        public List<string> GeneratePartition(List<string> partition)
        {
            ...
            for(int i = 0; i < partition.Count; i++)
            {
                ...
                for(int j = 0; j < states.Length; j++)
                {
                    ...
                    {
                        string[] trans = transitions[k].Split(',');
                        found = false;
                        for (int m = 0; m < partition.Count && !found; m++)
                        {
                            if (partition[m].Contains(trans[0]))
                            {
                                found = true;
